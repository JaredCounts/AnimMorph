...
{	
	...
	int currentIndex = 0;
	for (int colIndex = 0; colIndex < points.cols(); colIndex++)
	{
		const Vector2f &point = points.col(colIndex);
	...
	Matrix2Xf meshPoints = 0.1 * (Transform<float, 2, TransformTraits::Affine>(Translation2f(-average)) * points);

	Matrix2Xi edges(2, points.cols());
	for (int i = 0; i < points.cols(); i++)
	{
		edges(0, i) = i;
		edges(1, i) = (i + 1) % points.cols();
	}



	return Triangulate::Triangulate(edges, meshPoints);
